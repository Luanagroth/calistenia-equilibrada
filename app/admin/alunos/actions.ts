...
export async function extendStudentAccessAction(formData: FormData): Promise<void> {
  ...
  redirect("/admin/alunos?success=access-extended");
}

export async function blockStudentAction(formData: FormData): Promise<void> {
  const studentId = getStudentId(formData);
  await ensureAdmin();
  ...
  revalidatePath("/admin/alunos");
  redirect("/admin/alunos?success=student-blocked");
}
...
export async function reactivateStudentAction(formData: FormData): Promise<void> {
  ...
  const { error: accessError } = await supabaseAdmin.from("access_periods").insert({
    user_id: studentId,
    starts_at: now.toISOString(),
    ends_at: new Date(now.getTime() + 45 * 24 * 60 * 60 * 1000).toISOString(),
    status: "active",
    source: "manual_admin_reactivation",
    notes: "Acesso reativado pelo painel admin",
  });

  if (accessError) {
    throw new Error("Erro ao criar período de acesso na reativação.");
  }

  revalidatePath("/admin/alunos");
  redirect("/admin/alunos?success=student-reactivated");
}
