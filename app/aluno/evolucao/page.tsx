...
import { getEvolutionCheckins } from "@/lib/aluno/get-evolution-checkins";
...
function formatWeightChange(initialValue: number, latestValue: number) {
  ...
  if (Math.abs(delta) < 0.1) {
    return "Seu peso se manteve estavel nos registros.";
  }
...
function formatMobilityChange(initialValue: number, latestValue: number) {
  if (initialValue === latestValue) {
    return `Sua mobilidade se manteve estavel em ${formatLevelWithLabel(latestValue, mobilityOptions)}.`;
  }

  const baseMessage = `Sua mobilidade percebida saiu de ${formatLevelWithLabel(initialValue, mobilityOptions)} para ${formatLevelWithLabel(latestValue, mobilityOptions)}.`;

  return latestValue > initialValue
    ? `${baseMessage} Voce registrou melhora na mobilidade percebida.`
    : `${baseMessage} Sua mobilidade percebida ficou mais baixa nos registros mais recentes.`;
}

function formatEnergyChange(initialValue: number, latestValue: number) {
  if (initialValue === latestValue) {
    return `Sua disposicao se manteve parecida em ${formatLevelWithLabel(latestValue, energyOptions)}.`;
  }

  return latestValue > initialValue
    ? `Sua disposicao registrada melhorou de ${formatLevelWithLabel(initialValue, energyOptions)} para ${formatLevelWithLabel(latestValue, energyOptions)}.`
    : `Sua disposicao registrada caiu de ${formatLevelWithLabel(initialValue, energyOptions)} para ${formatLevelWithLabel(latestValue, energyOptions)}.`;
}

function formatPainChange(initialValue: number, latestValue: number) {
  if (initialValue === latestValue) {
    return `Seu desconforto se manteve estavel em ${formatLevelWithLabel(latestValue, painOptions)}.`;
  }
...
export default async function EvolucaoPage({
  ...
  const { profile, checkins, completedTrainings, journeyPercentage, trainingAverages } = evolution;
  ...
  const perceivedEvolutionMessages = [
    ...
    canCompareEnergy
      ...
      : "Sua disposicao passara a ser comparada assim que houver registros suficientes.",
    canComparePain
      ? formatPainChange(firstCheckin!.pain_level!, latestCheckin!.pain_level!)
      : "Seu desconforto geral podera ser comparado com mais clareza apos novos check-ins.",
  ];
  ...
  const summaryLead =
    checkins.length === 0
      ...
      : checkins.length === 1
        ...
        : "Comparando seu primeiro e ultimo check-in, ja da para perceber como sua evolucao percebida esta caminhando.";
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-xl text-slate-300">
            Acompanhe sua constancia, compare seu ponto de partida e veja como pequenas melhorias se acumulam ao longo da jornada.
          </p>
        </div>
        ...
        <Card className="border-emerald-400/20 bg-emerald-400/5">
          <CardContent className="flex items-center gap-4 p-5">
            ...
            <div className="space-y-1">
              ...
              <p className="text-xs text-slate-300">Seu registro ja entrou no resumo da sua evolucao percebida.</p>
            </div>
          </CardContent>
        </Card>
      )}
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <Card className="border-white/10 bg-[#10161A] shadow-2xl shadow-black/30">
          ...
          <CardContent>
            ...
            <Progress
              ...
              className="mt-3 h-2 bg-white/10 [&_[data-slot=progress-indicator]]:bg-yellow-400"
            />
          </CardContent>
        </Card>
        ...
        <Card className="border-white/10 bg-[#10161A] shadow-2xl shadow-black/30">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-sm font-medium text-slate-300">Treinos concluidos</CardTitle>
        ...
        <Card className="border-white/10 bg-[#10161A] shadow-2xl shadow-black/30">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-sm font-medium text-slate-300">Ultimo treino concluido</CardTitle>
            <Trophy className="h-4 w-4 text-yellow-400" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-white">
              {lastCompletedDay ? `Treino ${lastCompletedDay}` : "-"}
            </div>
        ...
        <Card className="border-white/10 bg-[#10161A] shadow-2xl shadow-black/30">
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-lg text-white">
              ...
              Check-in de evolucao
            </CardTitle>
            <p className="text-xs text-slate-400">
              Registre como voce esta hoje para acompanhar sua evolucao percebida ao longo da jornada.
            </p>
          </CardHeader>
          <CardContent>
            <form action={createEvolutionCheckinAction} className="space-y-5">
              <div className="grid gap-5 md:grid-cols-2">
                <div className="space-y-2">
                  <Label htmlFor="weightKg" className="text-xs text-slate-300">Peso atual</Label>
                  <Input
                    id="weightKg"
                    name="weightKg"
                    type="number"
                ...
                <div className="space-y-2 md:col-span-2">
                  <Label htmlFor="notes" className="text-xs text-slate-300">Observacoes</Label>
                  <Textarea
                    id="notes"
                    name="notes"
          ...
          <CardContent className="space-y-6">
            ...
            <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
              <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
                ...
                <p className="mt-3 text-2xl font-bold text-white">{completedTrainings}</p>
                <p className="text-xs text-slate-400">{journeyPercentage}% da jornada</p>
              </div>
              ...
              <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
                ...
                <p className="text-xs text-slate-400">Autorrelato nos treinos concluidos</p>
              </div>
            </div>
